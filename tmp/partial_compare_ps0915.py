...
出力: tmp/partial_compare_ps0915.txt
"""
import io
import json
import re
...
out = []
for l in io.open('tmp/split_built_ps0915.txt', encoding='utf-8').read().splitlines():
...
io.open('tmp/partial_compare_ps0915.txt', 'w', encoding='utf-8').write('\n'.join(out) + '\n')
print('%d件 → tmp/partial_compare_ps0915.txt' % sum(1 for x in out if x.startswith('new')))
