...
出力: tmp/compare_recheck_0916.md
"""
import datetime
import glob
import io
...
out = io.open('tmp/compare_recheck_0916.md', 'w', encoding='utf-8')
W = out.write
W('# 新着の独立再導出と登録値の突合（%s）\n\n' % TODAY)
...
out.close()
print('読めた %d件 / ズレあり %d件 → tmp/compare_recheck_0916.md' % (len(rows), diff_n))
