# -*- coding: utf-8 -*-
"""7/6 期限切れ→販売中 変換(構築側)。⚠️89件 + id689(Charツアー救済)を機械再パース。
買える枠ゼロ(None)=売切/終了=削除候補。tmp/convert_0706.json出力。"""
import sys, json, re
sys.path.insert(0, 'tools')
import build_pia_entries as bpe
from build_pia_entries import build

IDS = [int(x) for x in open('tmp/warn_ids_0706.txt', encoding='utf-8').read().split()]
if 689 not in IDS:
    IDS.append(689)
...
json.dump(out, open('tmp/convert_0706.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
