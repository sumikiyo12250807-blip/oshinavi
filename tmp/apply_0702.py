...
prop = json.load(open('tmp/convert_0702.json', encoding='utf-8'))
...
if DRY: print("(DRY)")
    ...
    open('index.html.bak_0702_morning_convert','w',encoding='utf-8').write(h)
    open('index.html','w',encoding='utf-8').write(h[:m.start()]+m.group(1)+new_arr+m.group(3)+h[m.end():])
    print("written (backup: index.html.bak_0702_morning_convert)")
