...
出力: tmp/partial_compare_ukother0915.txt
"""
import io
import json
import re
...
cands = {c['newid']: c for c in json.load(io.open('tmp/cands_ukother_0915.json', encoding='utf-8'))}
src = io.open('index.html', encoding='utf-8').read()
ev = {e['id']: e for e in json.loads(re.search(r'  const EVENTS = (\[.*?\]);', src, re.S).group(1))}
out = []
for l in io.open('tmp/split_built_ukother0915.txt', encoding='utf-8').read().splitlines():
...
io.open('tmp/partial_compare_ukother0915.txt', 'w', encoding='utf-8').write('\n'.join(out) + '\n')
print('%d件 → tmp/partial_compare_ukother0915.txt' % sum(1 for x in out if x.startswith('new')))
