# -*- coding: utf-8 -*-
"""新着プール(genre:"new")の振り分け下書きを作る（読むだけ・2026-09-13 朝）。
...
使い方: python tmp/assign_plan_0913.py
出力: tmp/assign_plan_0913.txt
"""
import io
import json
import re
...
for e in sorted(new, key=lambda x: x['id']):
    ...
    lines.append('%sid%-5s %-10s draft=%-10s map=%-10s %s | %s' % (
...
io.open('tmp/assign_plan_0913.txt', 'w', encoding='utf-8').write('\n'.join(head + lines) + '\n')
print('\n'.join(head[:2]))
