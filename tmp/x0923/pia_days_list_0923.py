# -*- coding: utf-8 -*-
"""ぴあの発売前一覧（指定ジャンル・先着0102/抽選0202）を全ページ読み、発売日が 9/22〜9/24 の行を全部出して、
OSHINAVIにその発売日の枠があるかを突き合わせる（読むだけ）。
使い方: python tmp/x0923/pia_days_list.py <lg>   例: 01=音楽 02=演劇 07=クラシック 06=イベント 03=スポーツ
...
DAYS = {'2026/9/24': '2026-09-24', '2026/9/25': '2026-09-25', '2026/9/26': '2026-09-26'}
...
for iso, r in sorted(hits, key=lambda x: x[0]):
    ...
    if not has:
        miss.append((iso, r, e))
print('ぴあ lg=%s で9/22〜9/24発売の行 %d件 ／ OSHINAVIにその日の枠が無い %d件' % (LG, len(hits), len(miss)))
for iso, r, e in miss:
