# -*- coding: utf-8 -*-
"""9/24(木)発売のX投稿の素材を作る（2026-09-23 夜）。
X_SCRIPT の本数の決め方＝①トレンド枠0〜1本 ②主役枠3本（Xフォロワー上位3組）③残りはジャンル別まとめ。
出力:
  tmp/x0923/material_top.md   … トレンド枠＋主役3本の素材（Fableに渡す）
  tmp/x0923/material.md       … まとめ枠の素材（ジャンル別・明日発売は全部／2〜3日後は大物5件＋丸めた件数）
...
STAR = [
    (4810, 'KiiiKiii', 149400, '@We_KiiiKiii（グローバル公式）／日本公式 @We_KiiiKiii_JP は11,300'),
    (4236, '東京スカパラダイスオーケストラ', 116500, '@tokyoskaj（2026-09-11実測）'),
    (18027, 'OWV', 59600, '@OWV_official'),
]
TREND = [8382, 7822]   # トレンド5位「#坂道好き顔9選」に当たる在庫
...
top = io.open('tmp/x0923/material_top.md', 'w', encoding='utf-8')
top.write('# 9/24(木)発売 X投稿の素材（トレンド枠＋主役3本）\n\n')
top.write('🚨ここに書いてある事実だけで書くこと。経歴・音楽的特徴・人気度・フォロワー数は書かない。\n')
top.write('🚨「先行」は条件を満たした人だけの枠＝一般発売と同じに書かない。\n\n')

top.write('## ① トレンド枠（Xトレンド5位「#坂道好き顔9選」× いま買える在庫）\n')
top.write('🚨トレンドの話題そのものには触れない。坂道グループのチケットが買えることだけ書く。\n\n')
for i in TREND:
    e = by_id.get(i)
    if not e:
        continue
...
top.write('## ②〜④ 主役枠（Xフォロワーが多い上位3組・9/24発売）\n\n')
for i, name, fo, note in STAR:
    ...
    if other:
        ...
        for t in other[:12]:
            top.write('    ・%s\n' % t.get('type'))
    top.write('\n')
# OWV は 15:00公演と19:00公演で別エントリ
e2 = by_id.get(18028)
if e2:
    top.write('※OWVは公演時間ごとに別エントリ＝id18028 もある\n')
    for t in slots_on(e2, TOM):
        top.write('    ・%s（%s）\n' % (t.get('type'), e2.get('dateLabel')))
top.close()

# まとめ枠
star_ids = {i for i, _, _, _ in STAR} | {18028} | set(TREND)
...
mat = io.open('tmp/x0923/material.md', 'w', encoding='utf-8')
mat.write('# 9/24(木)発売 X投稿の素材（ジャンル別まとめ枠）\n\n')
mat.write('🚨1投稿＝1ジャンル。明日発売はそのジャンルを**全部**載せる（一部だけだと「うちの推しは無い」と閉じられる）。\n')
mat.write('🚨1行＝1アーティスト（同じ時刻の地域は「・」でまとめる）。時刻を行頭に置く。\n')
mat.write('🚨主役3組とトレンド枠の2組はここから外してある（別の投稿で出すため）。\n\n')
...
mat.close()
print('wrote tmp/x0923/material_top.md, tmp/x0923/material.md')
