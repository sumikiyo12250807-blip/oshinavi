...
→ **足すだけにする**。既存の枠は1つも消さない。増えた分だけ append する。
   減る方向の差分は「消す」のではなく**報告して人が見る**（tmp/merge_shrink_0822.txt）。

日付・会場も**伸びる方向にだけ**更新する（千秋楽が後ろに伸びた時だけ date を更新）。

使い方:
  python tmp/merge_apply_0822.py            # 差分を見るだけ
  python tmp/merge_apply_0822.py --apply
"""
import io
import json
import re
...
built = {e['id']: e for e in json.load(open('tmp/merge_built_0822.json', encoding='utf-8'))}
...
log = io.open('tmp/merge_diff_0822.txt', 'w', encoding='utf-8')
shrink = io.open('tmp/merge_shrink_0822.txt', 'w', encoding='utf-8')
...
log.close()
shrink.close()
print('枠が増えたエントリ %d件 / 追加した枠 %d → tmp/merge_diff_0822.txt' % (touched, added_total))
print('※再導出のほうが少なかった分は消さずに tmp/merge_shrink_0822.txt に出した')

if APPLY:
    shutil.copyfile('index.html', 'index.html.bak_0822_merge')
    open('index.html', 'w', encoding='utf-8').write(
        h[:m.start()] + m.group(1) + json.dumps(EVENTS, ensure_ascii=False, indent=2)
        + m.group(3) + h[m.end():])
    print('適用した（backup: index.html.bak_0822_merge）')
else:
