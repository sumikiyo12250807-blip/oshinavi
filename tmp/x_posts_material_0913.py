# -*- coding: utf-8 -*-
"""9/12夜のX投稿（明日9/13発売）の素材を、投稿1本ずつに割って1枚にする。Fableに渡すのはこれと X_SCRIPT.md だけ。
・明日9/13発売＝その投稿のジャンルを全部（同じ時刻・名前・県・先行の行は1行にまとめる）
・9/14・9/15＝その投稿のジャンルから箱の大きい順に5件、残りは「他にも◯件以上」（10の位で切り下げ・10未満はそのまま）
🚨「発売開始」＝ticket.startDate がその日のもの。新着プール（振り分け前）は出さない。売り切れ枠は出さない。
出力: tmp/x0913/material.md
"""
...
o = io.open('tmp/x0913/material.md', 'w', encoding='utf-8')
W = o.write
