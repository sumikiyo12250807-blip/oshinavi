...
出力: tmp/cands_<出力名>_0918.json（build_pia_entries にそのまま渡せる形）＋ .txt
"""
import glob
import io
import json
...
json.dump(out, io.open('tmp/cands_%s_0918.json' % TAG, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
io.open('tmp/cands_%s_0918.txt' % TAG, 'w', encoding='utf-8').write('\n'.join(lines) + '\n')
print('読んだファイル %d本 / 本当に未登録の候補 %d件（名前×公演日で登録済み %d件を外した）→ tmp/cands_%s_0918.json' % (
