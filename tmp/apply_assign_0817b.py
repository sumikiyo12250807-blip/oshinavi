# -*- coding: utf-8 -*-
"""新着プールの振り分けを適用する。
決定表 tmp/decision_0817b.json = {"assign": {"4426": "jpop", ...}, "hold": [4377, ...]}
...
if APPLY:
    io.open('index.html.bak_0817b_assign', 'w', encoding='utf-8', newline='').write(
        io.open('index.html', encoding='utf-8', newline='').read())
    io.open('index.html', 'w', encoding='utf-8', newline='').write(src)
    print('\n適用しました（backup: index.html.bak_0817b_assign）')
else:
    print('\n（判定のみ。適用するなら --apply）')
