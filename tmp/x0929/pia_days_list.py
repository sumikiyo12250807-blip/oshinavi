# -*- coding: utf-8 -*-
"""ぴあの発売前一覧（指定ジャンル・先着0102/抽選0202）を全ページ読み、発売日が 9/29〜10/1 の行を全部出して、
OSHINAVIにその発売日の枠があるかを突き合わせる（読むだけ）。
使い方: python tmp/x0929/pia_days_list.py <lg>   例: 01=音楽 02=演劇 07=クラシック 06=イベント 03=スポーツ
...
DAYS = {'2026/9/29': '2026-09-29', '2026/9/30': '2026-09-30', '2026/10/1': '2026-10-01'}
...
for iso, r in sorted(hits, key=lambda x: x[0]):
    ...
    if not has:
        miss.append((iso, r, e))
print('ぴあ lg=%s で9/29〜10/1発売の行 %d件 ／ OSHINAVIにその日の枠が無い %d件' % (LG, len(hits), len(miss)))
for iso, r, e in miss:
