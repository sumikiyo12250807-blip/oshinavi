...
d = json.load(io.open('tmp/batch_cand_0829c.json', encoding='utf-8'))
...
io.open('tmp/merge_todo_0829c.json', 'w', encoding='utf-8').write(json.dumps(full, ensure_ascii=False, indent=1))
o = io.open('tmp/dup_part_0829c.md', 'w', encoding='utf-8')
o.write('# 2026-08-28 部分一致で統合待ち（機械で決めきれない＝目視候補）%d件\n\n' % len(part))
...
print('部分一致(目視候補): %d件 → tmp/dup_part_0829c.md' % len(part))
