...
DRAFT = "tmp/pickup0920/draft.md"
OUT = "tmp/pickup0920/section.html"
FROM, TO = "2026-09-21", "2026-09-27"
WD = "月火水木金土日"
...
B = ['<section class="pickup" id="pickup">',
     ...
     '  <p class="pk-sub">9/21(月・祝)〜9/27(日)にチケットの発売が始まるアーティスト紹介</p>',
     '  <div class="pk-lede">']
...
for n, (name, ids) in enumerate(MAIN):
    ...
    if name.startswith("劇団四季"):
        ...
        B.append(FIG_CHORUS)
    B.append(br([p for p in paras_of(body) if not p.startswith("ほかにも、")]))
    for i in ids:
        e = by_id[i]
