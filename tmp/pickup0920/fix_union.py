...
安全弁が止めたのは、取り直し（tmp/pickup0920/fix_asheal.json）に元の生きた枠が入っていなかったから。
...
ids = [int(x) for x in io.open('tmp/pickup0920/fix_ids.txt', encoding='utf-8').read().split(',') if x.strip()]
src = io.open('index.html', encoding='utf-8', newline='').read()
m = re.search(r'(  const EVENTS = )(\[.*?\])(;)', src, re.S)
events = json.loads(m.group(2))
by = {e['id']: e for e in events}
built = {o['id']: o for o in json.load(io.open('tmp/pickup0920/fix_asheal.json', encoding='utf-8')) if o.get('status') == 'convert'}
