...
OVERRIDE = {}
NOTE = ('新着プールのぴあ由来（9/16 に入れた66件）。別エージェント1本がぴあの実ページから公演・枠・ジャンル表記をゼロで読み直し、'
        '登録と食い違いが無く、ジャンルもページのジャンル番号で確かめられた分だけ（9/17朝・'
        'ズレ2件（10804 DIAURA の締切時刻・10835 プリキュアの売り切れ回）は登録の型どおりと確認）。買える枠0・読めなかった・ジャンルを確かめ切れなかった分はプールに残した。')

APPLY = '--apply' in sys.argv
OK = {int(x) for x in re.findall(r'\d+', io.open('tmp/assign_ok_ids_0917.txt', encoding='utf-8').read())}
