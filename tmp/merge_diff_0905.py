# -*- coding: utf-8 -*-
"""統合適用の前後で「画面に出る枠」が減ったエントリを数える。
出典 feedback_heal_flattens_ticket_types（阪神×広島 12枠→1枠を機械ゲートが全部素通りさせた）。

比較の相手は HEAD ではなく **統合直前のバックアップ** にする。
今朝は先に10件削除しているので HEAD と比べるとその分が差として出てしまう。
"""
import re, json, sys, subprocess
sys.stdout.reconfigure(encoding='utf-8')
TODAY = '2026-09-05'
BEFORE = 'index.html.bak_0905_merge'   # heal --apply が取ったバックアップ
...
print(f'エントリ数  統合前 {len(old)} → 後 {len(new)}')
