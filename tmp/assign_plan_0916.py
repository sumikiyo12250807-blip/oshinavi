# -*- coding: utf-8 -*-
"""新着プール(genre:"new")の振り分け下書きを作る（読むだけ・2026-09-15 朝）。assign_plan_0914.py の日付違い。
...
使い方: python tmp/assign_plan_0916.py
出力: tmp/assign_plan_0916.txt
"""
import collections
import io
import json
...
head = [
    ...
    '',
    '--- ぴあの区分 → 下書き の組み合わせ（件数順）---',
] + ['  %4d  %s → %s' % (n, s or '(なし)', d) for (s, d), n in subs.most_common()] + ['', '--- 1件ずつ ---']
io.open('tmp/assign_plan_0916.txt', 'w', encoding='utf-8').write('\n'.join(head + lines) + '\n')
print('\n'.join(head[:2]))
