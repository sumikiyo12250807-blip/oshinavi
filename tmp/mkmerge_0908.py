...
出力＝tmp/merge_in_0908.json（merge_apply_0905.py に渡す）と、目で見るための .md
"""
import io, re, json, datetime, collections

# 候補id → 足し先の既存id（エージェントの独立判定 B22件）
PAIRS = [
    (7193, 4227), (7203, 2405), (7204, 2405), (7205, 4802), (7207, 1202),
    (7210, 4499), (7212, 6972), (7218, 3483), (7249, 5413), (7260, 5430),
    (7263, 2411), (7264, 6508), (7279, 5755), (7280, 5755), (7282, 2564),
    (7283, 2564), (7284, 1273), (7285, 1273), (7286, 1273), (7327, 1202),
    (7330, 6096), (7331, 6096),
    (7117, 1202),   # 既存どうし＝同じ「鶴フェスへの道」ツアーが2エントリに割れていた
]
...
VENUE_FIX = {1202: ("全国ツアー（米子laughs／松江canova／広島Cave-Be）", "鳥取・島根・広島")}
...
for _i in (7117,):
    if _i not in cand and _i in EV:
        cand[_i] = EV[_i]
srcurl = {c["newid"]: c["urls"][0] for c in json.load(io.open("tmp/cand_0908.json", encoding="utf-8"))}
...
json.dump(out, io.open("tmp/merge_in_0908.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)

o = io.open("tmp/merge_in_0908.md", "w", encoding="utf-8")
...
if warns:
    ...
    for w in warns:
        o.write("- %s\n" % w)
o.close()
print("targets=%d tickets=%d warns=%d -> tmp/merge_in_0908.json / .md"
