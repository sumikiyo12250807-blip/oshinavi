# -*- coding: utf-8 -*-
"""楽天の発売前ハーベスト（tmp/rakuten_presale.json の presale）が、うちに登録済みかを当てる（読むだけ・2026-09-15）。
rakuten_presale_check_0914.py の出力名違い。
判定＝楽天の公演コード（URL末尾の /rtXXXX/）が index.html のどこかに出てくるか（Deep Link は murl にエンコードされて入る）。
登録済みなら、そのエントリに「発売前の枠」が入っているか（startDate が今日より後の枠）も見る＝既存への足し込み漏れを拾う。
使い方: python tmp/rakuten_presale_check_0915.py
出力: tmp/rakuten_presale_check_0915.txt
"""
import datetime
import io
import json
...
io.open('tmp/rakuten_presale_check_0915.txt', 'w', encoding='utf-8').write('\n'.join(out) + '\n')
print('\n'.join(out))
