# -*- coding: utf-8 -*-
"""X投稿の機械検品（9/15夜・9/16発売ぶん）。tmp/x0916/post*.txt のうち 9/16 の見出しのものだけを見る
（同じフォルダに 前の日の分が残っていることがあるので、見出しで分ける）。
見るもの＝台本 X_SCRIPT.md と memory の封印語・癖（feedback_x_user_edited_samples ほか）。
使い方: python tmp/x0916/check_posts.py
"""
import datetime, glob, io, re, sys
sys.stdout.reconfigure(encoding='utf-8')
WD = '月火水木金土日'
HEAD = 'OSHINAVIの"9/16チケット発売"ピックアップ🎫'
TAGS = '#OSHINAVI #明日発売 #チケット'
BAN = ['生で浴び', 'あんた', '押さえ', '迷うなら両方', 'さっきの投稿', 'ひとつ前']
SOFT = ['ぶん', '動く', '動き', '開く', '開い', '枠が', '受付が', 'が全部', 'こそが', 'というもの', 'しなさい', 'しまいなさい']
files = [f for f in sorted(glob.glob('tmp/x0916/post*.txt'))
         if io.open(f, encoding='utf-8-sig').read().splitlines()[:1] == [HEAD]]
skipped = [f for f in sorted(glob.glob('tmp/x0916/post*.txt')) if f not in files]
bad = 0
phr = {}
for f in files:
...
if skipped:
    print('\n（9/16 の見出しでないので見ていない: %s）' % ' '.join(f[-10:] for f in skipped))
print('\nNG合計 %d' % bad)
