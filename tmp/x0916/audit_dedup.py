# -*- coding: utf-8 -*-
"""X投稿のライブの総ざらい（audit_posts.txt・audit_posts2.txt）の「未登録」候補から、見かけの取りこぼしを外す（読むだけ・2026-09-15夜）。
...
使い方: python tmp/x0916/audit_dedup.py
出力: tmp/x0916/audit_dedup.txt（本当の抜け／登録済みらしい）と tmp/x0916/audit_real_missing.json（本当の抜けのURL）
"""
import io
import json
import re
...
cands = parse('tmp/x0916/audit_posts.txt') + parse('tmp/x0916/audit_posts2.txt')
seen, uniq = set(), []
for c in cands:
...
o = io.open('tmp/x0916/audit_dedup.txt', 'w', encoding='utf-8')
...
o.close()
io.open('tmp/x0916/audit_real_missing.json', 'w', encoding='utf-8').write(
    json.dumps([c for c, _ in real], ensure_ascii=False, indent=1))
print('候補 %d ＝ 本当の抜け %d ／ 登録済みらしい %d → tmp/x0916/audit_dedup.txt' % (len(uniq), len(real), len(dup)))
