# dateLabel（画面に出る公演期間）と tickets の公演日が食い違うエントリを検出する（改良版）
#  - ラベルの「M月D日」は直前に出た年を引き継ぐ
#  - ticket の公演日の年は「締切(ticket.date)に一番近い年」を選ぶ（締切は公演直前なので当たる）
...
def show_dates(e):
    ...
    for t in e.get('tickets',[]):
        ...
        for m in re.finditer(r'(R9年\s*)?(\d{1,2})/(\d{1,2})',ty):
...
for e in ev:
    ...
    if over>1 or under>1:
        bad.append((e['id'],e.get('artist','')[:30],e['date'],e.get('dateLabel','')[:46],
                    f"{smin}〜{smax}",over,under))
print('dateLabel が実際の公演日をカバーしていないエントリ:',len(bad),'件 / 全',len(ev))
bad.sort(key=lambda x:-(x[5]+x[6]))
for b in bad[:40]:
    print(f"  id={b[0]:5} {b[1]:32} date={b[2]} label[{b[3]}] 実公演={b[4]} 後ろ超過{b[5]}日 前超過{b[6]}日")
