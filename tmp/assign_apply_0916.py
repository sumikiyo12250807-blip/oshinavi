...
OVERRIDE = {}
NOTE = ('新着プールのぴあ由来（9/14・9/15 に入れた分）。別エージェント5本がぴあの実ページから公演・枠・ジャンル表記をゼロで読み直し、'
        '登録と食い違いが無く、ジャンルもページのジャンル番号で確かめられた分だけ（tmp/compare_recheck_0916.md・'
        'tmp/assign_ok_build_0916.py）。買える枠0・読めなかった・ジャンルを確かめ切れなかった分はプールに残した。')

APPLY = '--apply' in sys.argv
OK = {int(x) for x in re.findall(r'\d+', io.open('tmp/assign_ok_ids_0916.txt', encoding='utf-8').read())}
