# -*- coding: utf-8 -*-
"""今日(2026-09-21)発売の枠の発売時刻を全部出す。昼ヒールの実行時刻を決めるため。"""
...
out = io.open("tmp/today_saletimes_0921.txt", "w", encoding="utf-8")
...
for r in sorted(rows):
    out.write("%s  id%s  %s\n" % r)
out.close()
print("wrote tmp/today_saletimes_0921.txt  rows=%d  slots=%s" % (len(rows), sorted(times.items())))
