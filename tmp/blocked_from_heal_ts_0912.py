...
見送られた分は tmp/heal_ids.json に convert として残っているので、
...
log = open('tmp/heal_apply_today_started_0912.txt', encoding='utf-8').read()
sec = log.split('🛡️')[1] if '🛡️' in log else ''
sec = sec.split('🚨🚨')[0]
ids = [int(x) for x in re.findall(r'id=(\d+) ', sec)]

built = json.load(open('tmp/heal_ids.json', encoding='utf-8'))
...
json.dump(out, open('tmp/blocked_built_ts_0912.json', 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1)
print('ブロックされた %d件 → ビルド結果が取れた %d件 → tmp/blocked_built_ts_0912.json'
      % (len(ids), len(out)))
