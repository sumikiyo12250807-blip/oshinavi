...
rows = []
per_file = []
for p in sorted(glob.glob("tmp/sweep_0908/*.json")):
    try:
...
o = io.open("tmp/sweep_count_0908.md", "w", encoding="utf-8")
W = o.write
W("# 9/8 発売前スイープの棚卸し\n\n")
W("| ファイル | 未掲載候補 |\n|---|---|\n")
...
o.close()
json.dump(list(uniq.values()), io.open("tmp/sweep_cand_0908.json", "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("total=%d uniq=%d -> tmp/sweep_count_0908.md / tmp/sweep_cand_0908.json"
      % (len(rows), len(uniq)))
