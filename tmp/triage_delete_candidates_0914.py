...
使い方: python tmp/triage_delete_candidates_0914.py
出力: tmp/delete_candidates_0914.md ＋ tmp/delete_candidates_piaonly_0914.txt（idのカンマ区切り）
"""
import datetime
import json
import re
...
with open('tmp/delete_candidates_0914.md', 'w', encoding='utf-8') as f:
    ...
    for k in ('他社あり', '売切/終了フラグあり', '締切不明あり', 'ぴあだけ・要他社確認', '既に消えている'):
        ...
        for i, n, u, d in sorted(rows):
            ...
            if u:
                f.write('  - %s\n' % u)
open('tmp/delete_candidates_piaonly_0914.txt', 'w', encoding='utf-8').write(
    ','.join(str(r[0]) for r in sorted(groups['ぴあだけ・要他社確認'])))
print('買える枠ゼロ %d件を仕分けた → tmp/delete_candidates_0914.md' % len(cands))
for k in groups:
