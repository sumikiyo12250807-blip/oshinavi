# -*- coding: utf-8 -*-
"""「〆切日に発売時刻がくっつく」型を探す（読むだけ・2026-09-14 朝）。today_started_0912.py の拡張。
条件＝ startDate が今日以前 かつ date > 今日 かつ 券種名が「M/D HH:MM発売」で終わる（かつ売り切れでない）。
0912版は startDate == 今日 だけを見ていた＝前日以前に発売した枠の取り残しも拾えるように「以前」に広げた。
...
🚨発売時刻がまだ来ていない枠（今日の発売で時刻前）は、ぴあもまだ発売前＝取り直しても変わらない。
使い方: python tmp/today_started_0914.py
出力: tmp/today_started_ids_0914.txt（発売時刻を過ぎた枠を持つid）
"""
import datetime
import io
import json
...
for e in ev:
    for t in e.get('tickets') or []:
        ...
        bucket.setdefault(e['id'], []).append('%s（発売 %s／締切 %s）' % (t.get('type'), sd, t.get('date')))
...
io.open('tmp/today_started_ids_0914.txt', 'w', encoding='utf-8').write(','.join(str(i) for i in sorted(past)))
