# -*- coding: utf-8 -*-
"""push前の抜き取り（tmp/push_sample_noon_0912.txt の id）について、朝のpush以降に既存エントリに増えた・書き換わった枠
（tmp/added_windows_noon_0912.txt）が、別エージェントがぴあ実ページからゼロで読んだ枠（scratchpad/push_check_noon_result.json）と
合っているかを見る（読むだけ）。
...
ids = {int(ln.split('\t')[0]) for ln in io.open('tmp/push_sample_noon_0912.txt', encoding='utf-8') if ln.strip()}
added = []
