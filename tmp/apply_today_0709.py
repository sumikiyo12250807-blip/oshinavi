...
built = {o['id']: o for o in json.load(open('tmp/convert_today_0709.json', encoding='utf-8'))}
...
if DRY:
    ...
    open('index.html.bak_0709_today_sale','w',encoding='utf-8').write(h)
    open('index.html','w',encoding='utf-8').write(h[:m.start()]+m.group(1)+new_arr+m.group(3)+h[m.end():])
    print("written (backup: index.html.bak_0709_today_sale)")
