...
出力: tmp/compare_recheck_0911.md
"""
import datetime
import glob
import io
...
SP = r'C:\Users\user\AppData\Local\Temp\claude\C--Users-user-oshinavi\5c61d24f-1d45-418a-9e82-a4c0b643a306\scratchpad'
...
out = io.open('tmp/compare_recheck_0911.md', 'w', encoding='utf-8')
W = out.write
W('# 新着の独立再導出と登録値の突合（%s）\n\n' % TODAY)
diff_n = 0
...
W('\n読めた %d件 / ズレあり %d件\n' % (len(rows), diff_n))
out.close()
print('読めた %d件 / ズレあり %d件 → tmp/compare_recheck_0911.md' % (len(rows), diff_n))
