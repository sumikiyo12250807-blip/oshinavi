...
使い方: python tmp/x0918/material_0918.py
出力: tmp/x0918/material.md
"""
import collections
import datetime
import io
...
o = io.open('tmp/x0918/material.md', 'w', encoding='utf-8')
W = o.write
W('# 9/17夜のX投稿の素材（明日 9/18(金) 発売）\n\n')
    ...
    for di, d in enumerate(DAYS):
        ...
        if di > 0:
            ...
            if mm:
                W('（%s）\n' % mm)
o.close()
print('→ tmp/x0918/material.md')
print('9/17発売の行（新着を除く）: ' + ' '.join('%s%d' % (g, n) for (d, g), n in sorted(genres_seen.items()) if d == DAYS[0]))
if stray:
