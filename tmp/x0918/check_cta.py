...
使い方: python tmp/x0918/check_cta.py
"""
import glob
import io
import json
...
out = []
for path in sorted(glob.glob('tmp/x0918/post*.txt')):
...
io.open('tmp/x0918/check_cta.txt', 'w', encoding='utf-8').write('\n'.join(out) + '\n')
print('wrote tmp/x0918/check_cta.txt (%d lines)' % len(out))
