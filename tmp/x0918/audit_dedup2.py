...
出力: tmp/x0918/audit_dedup2.txt（本当の抜け／登録済みらしい）と tmp/x0918/audit_real_missing2.json（本当の抜けのURL）
"""
import io
import json
import re
...
cands = parse('tmp/x0918/audit_posts2.txt')
seen, uniq = set(), []
for c in cands:
...
o = io.open('tmp/x0918/audit_dedup2.txt', 'w', encoding='utf-8')
...
o.close()
io.open('tmp/x0918/audit_real_missing2.json', 'w', encoding='utf-8').write(
    json.dumps([c for c, _ in real], ensure_ascii=False, indent=1))
print('候補 %d ＝ 本当の抜け %d ／ 登録済みらしい %d → tmp/x0918/audit_dedup2.txt' % (len(uniq), len(real), len(dup)))
