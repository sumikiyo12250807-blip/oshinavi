# -*- coding: utf-8 -*-
"""まとめ直した投稿のリストが、素材（tmp/x0918/material.md）の行を1つも落としていないか確かめる（読むだけ・9/15夜の便）。
...
mat = io.open('tmp/x0918/material.md', encoding='utf-8').read()
posts = ''.join(io.open(f, encoding='utf-8-sig').read() + '\n' for f in sorted(glob.glob('tmp/x0918/post0[4-9].txt')))
