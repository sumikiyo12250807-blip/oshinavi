...
MAIN = [("MONO NO AWARE", 4500), ("ASKA", 4489), ("東京スカパラダイスオーケストラ", 4236),
        ("佐藤竹善", 668), ("湖月わたる", 4246)]
# 🚨2026-09-06 差し替え＝「載せるべきなのに載っていない大物」がいた。
# この窓(9/7〜9/13)で発売が始まるのは356件あるのに、記事は17組。
# 前のタイルはブギ連・HIZAKI・Sick2 などで、沢田研二・モーニング娘。'26・徳永英明が抜けていた。
# 選ぶ前に窓の発売を全部並べる＝tmp/pickup_gap_0906.py（来週から手順に入れる）。
# 名前とidは tmp/tiles_cand_0906.py で index.html から機械抽出したもの（手で写さない）。
TILES = [("沢田研二", 3118), ("モーニング娘。’26", 3422), ("徳永英明", 3053),
         ("矢野顕子", 4103), ("綾小路きみまろ", 1109), ("Juice=Juice", 4538),
         ("山内惠介", 177), ("ハリー・ポッターTM in コンサート", 5664),
         ("島津亜矢", 710), ("スターダスト☆レビュー", 4496),
         ("THE BAWDIES", 4230), ("令和八年 冬巡業 大相撲三田場所", 6141)]
