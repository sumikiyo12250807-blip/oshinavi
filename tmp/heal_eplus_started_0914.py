# -*- coding: utf-8 -*-
"""e+ の「〆切日に発売時刻がくっつく」枠を、e+ の実ページから締切を取り直して直す（2026-09-14・ぴあのヒールの e+ 版）。

対象＝tmp/today_started_eplus_0914.txt の78件のうち、
  発売日（startDate）が今日以前・締切（date）が未来・券種名が「…（…公演）9/12 10:00発売」で終わる e+ の枠。
  画面はこの形を「〜締切日 発売時刻」と出す＝**締切の時刻が嘘**（発売の時刻が入っている）。
  tools/fix_eplus_deadlines.py は「今日」が 7/22 固定・新着だけ・startDate のある枠を飛ばすので使えない。
...
当て方＝枠自身の -P URL を1回ずつ読み、受付期間の「開始日・開始時刻」がその枠の startDate・発売時刻と同じ窓を探す。
  ...
  ・見つからない／決まらない／読めない → 触らずに一覧に出す
e+ は叩きすぎると 503 で黙って壊れる＝1ページずつ1秒あける。
使い方: python tmp/heal_eplus_started_0914.py [--apply]
出力: tmp/heal_eplus_started_0914.md
"""
import datetime
import io
import json
...
IDS = {int(x) for x in io.open('tmp/today_started_eplus_0914.txt', encoding='utf-8').read().split(',') if x.strip()}
...
out = io.open('tmp/heal_eplus_started_0914.md', 'w', encoding='utf-8')
W = out.write
...
for k, rows in res.items():
    ...
    W('\n')
out.close()
print(' / '.join('%s %d' % (k, len(v)) for k, v in res.items()), '→ tmp/heal_eplus_started_0914.md')
if not APPLY:
    print('(--apply で書き込み)')
    sys.exit(0)
nl = '\r\n' if '\r\n' in src else '\n'
