...
index.html の登録値を突き合わせる（読むだけ）。compare_recheck_0912.py の日付違い。
...
見るもの:
 ...
 ⑤読めなかった件（note）
使い方: python tmp/compare_recheck_0914.py
出力: tmp/compare_recheck_0914.md
"""
import datetime
import glob
import io
...
out = io.open('tmp/compare_recheck_0914.md', 'w', encoding='utf-8')
W = out.write
W('# 新着の独立再導出と登録値の突合（%s）\n\n' % TODAY)
diff_n = 0
...
W('\n読めた %d件 / ズレあり %d件\n' % (len(rows), diff_n))
out.close()
print('読めた %d件 / ズレあり %d件 → tmp/compare_recheck_0914.md' % (len(rows), diff_n))
