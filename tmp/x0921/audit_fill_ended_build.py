# -*- coding: utf-8 -*-
"""取りこぼし24件のうち「ぴあで買える枠0」だった18件を、印付きの枠として組み立てる（2026-09-21）。
...
出力: tmp/x0921/audit_fill_ended_built.json（1候補＝1URL・各ticketに url を付ける）
"""
...
SKIP = {950012, 950013, 950016, 950017, 950021, 950023}   # 前回入れた6件
...
cands = [c for c in json.load(io.open('tmp/x0921/audit_fill_cands.json', encoding='utf-8')) if c['newid'] not in SKIP]
out = []
for c in cands:
    e = B.build(c)
    if not e:
...
json.dump(out, io.open('tmp/x0921/audit_fill_ended_built.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
