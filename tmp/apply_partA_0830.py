...
def key(t):
    ...
    return (t.get('type'),u)
reb={e['id']:e for e in json.load(io.open('tmp/built_partA2_0830.json',encoding='utf-8'))}
P='index.html'
shutil.copy(P,'index.html.bak_0830_partA')
src=io.open(P,encoding='utf-8',newline='').read()
m=re.search(r'(  const EVENTS = )(\[.*?\])(;)',src,re.S)
EVENTS=json.loads(m.group(2))
log=io.open('logs/merged_partA_2026-08-30.md','w',encoding='utf-8')
log.write('# 2026-08-30 部分一致の判定で「同じ興行」と確定した5件の統合（追加のみ・置換なし）\n\n')
nent=nslot=ndate=0
for e in EVENTS:
    b=reb.get(e['id'])
    if not b: continue
...
log.close()
print('統合 %d エントリ / 追加 %d 枠 / 千秋楽が伸びた %d件 → logs/merged_partA_2026-08-30.md'%(nent,nslot,ndate))
