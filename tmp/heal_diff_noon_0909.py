# -*- coding: utf-8 -*-
"""ヒール適用の前後で「画面に出る枠」が減ったエントリを数える（+ 会期の縮みも見る）。
...
TODAY = "2026-09-09"
BEFORE = "index.html.bak_0909_prenoonheal"
AFTER = "index.html"
OUT = "tmp/heal_diff_noon_0909.txt"
...
for eid, ea in A.items():
    ...
    # 会期（公演日・ラベル）が縮んでいないか
    da, db = ea.get("date") or "", eb.get("date") or ""
    la, lb = ea.get("dateLabel") or "", eb.get("dateLabel") or ""
    if (db and da and db < da) or (la != lb):
        shrunk.append((eid, ea.get("name", "")[:36], da, db, la[:44], lb[:44]))
...
with io.open(OUT, "w", encoding="utf-8") as f:
    f.write("=== 昼ヒール適用の前後・画面に出る枠の突合 (today=%s) ===\n" % TODAY)
    f.write("前 %d枠 → 後 %d枠 （差 %+d）\n" % (tot_a, tot_b, tot_b - tot_a))
    f.write("枠が消えたエントリ %d件 / 枠が増えたエントリ %d件 / 会期が動いた %d件\n\n"
    ...
    f.write("\n【会期(date/dateLabel)が動いたエントリ %d件】\n" % len(shrunk))
    for eid, name, da, db, la, lb in shrunk:
        f.write("  id=%-6s %s\n      date %s → %s\n      label %s → %s\n" % (eid, name, da, db, la, lb))
    f.write("\n【枠が増えたエントリ %d件】\n" % len(gained))
    for eid, name, na, nb in gained[:80]:
        f.write("  id=%-6s %s  %d枠→%d枠\n" % (eid, name, na, nb))

print("前=%d 後=%d 差=%+d / 消えた%d件 増えた%d件 会期が動いた%d件"
      % (tot_a, tot_b, tot_b - tot_a, len(lost), len(gained), len(shrunk)))
