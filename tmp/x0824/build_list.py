# -*- coding: utf-8 -*-
"""8/24発売の全52件を、ジャンル別の「全部のせ」投稿に組む（2026-08-22 ユーザー指示）。
...
NL = chr(10)
out = io.open('tmp/x0824/list_posts.txt', 'w', encoding='utf-8')
GROUPS = [g for g in GROUPS if any(r['genre'] in g[1] for r in rows)]
n = len(GROUPS)
total = 0
for i, (label, gs, emoji) in enumerate(GROUPS, 1):
    ...
    out.write('=== (%d/%d) %s : %d件 / %d字 ===' % (i, n, label, len(items), len(body)) + NL + body + NL + NL)
out.write('合計 %d件' % total + NL)
out.close()
print('合計 %d件 → tmp/x0824/list_posts.txt' % total)
