# -*- coding: utf-8 -*-
"""9/16夜のX投稿（明日 9/17 発売）の素材を、投稿1本ずつに割って1枚にする。Fableに渡すのはこれと X_SCRIPT.md だけ。
tmp/x0914_0913night/x_posts_material_0914.py の日付違い＋「他にも◯件」の丸め方を今の決まり（X_SCRIPT.md 3番・2026-09-13）に直した版。
・明日9/17発売＝その投稿のジャンルを全部（同じ時刻・名前・県・先行の行は1行にまとめる）
・9/18・9/19＝その投稿のジャンルから箱の大きい順に5件、残りは四捨五入＝下1桁0〜4なら「◯件以上」／5〜9なら「◯件近く」／10未満はそのまま
🚨「発売開始」＝ticket.startDate がその日のもの。新着プール（振り分け前）は出さない。売り切れ枠は出さない。
使い方: python tmp/x0917/material_0916.py
出力: tmp/x0917/material.md
"""
import collections
import datetime
import io
...
o = io.open('tmp/x0917/material.md', 'w', encoding='utf-8')
W = o.write
W('# 9/16夜のX投稿の素材（明日 9/17(木) 発売）\n\n')
