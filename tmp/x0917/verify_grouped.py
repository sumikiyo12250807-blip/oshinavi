# -*- coding: utf-8 -*-
"""まとめ直した投稿のリストが、素材（tmp/x0917/material.md）の行を1つも落としていないか確かめる（読むだけ・9/15夜の便）。
素材の各行（時刻・名前・県・先行）を、投稿側の「時刻 名前／県・県…」に展開したものと突き合わせる。
まとめる規則（ALIAS・LINE）は tmp/x0913/group_lists.py と同じものを使う。
使い方: python tmp/x0916/verify_grouped.py
"""
...
mat = io.open('tmp/x0917/material.md', encoding='utf-8').read()
posts = ''.join(io.open(f, encoding='utf-8-sig').read() + '\n' for f in sorted(glob.glob('tmp/x0917/post0[4-9].txt')))
A = rows_of(mat.splitlines())
B = rows_of(posts.splitlines())
lost = sorted(A - B)
extra = sorted(B - A)
print('素材の行（県ごと）%d ／ 投稿の行（県ごと）%d ／ 投稿に無い %d ／ 素材に無い %d' % (len(A), len(B), len(lost), len(extra)))
for x in lost:
    print('  投稿に無い: %s %s／%s%s' % x)
for x in extra:
    print('  素材に無い: %s %s／%s%s' % x)
