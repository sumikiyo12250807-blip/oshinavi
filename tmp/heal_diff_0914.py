...
heal_diff_0913.py の日付違い。ヒール自身の安全弁は公演単位でしか比べないので、同じ公演の券種違い
（阪神の車椅子席など）が丸ごと消えても気づかない。だから HEAD と現物を突き合わせる。

使い方: python tmp/heal_diff_0914.py [比較元のgitリビジョン(既定HEAD)]
出力: tmp/heal_diff_0914.txt
"""
import datetime
import io
import json
...
head = ['=== ヒール前後で画面から消えた枠（比較元 %s・today=%s）===' % (REV, TODAY),
        '消えたエントリ %d件 / 消えた枠 %d本' % (len([l for l in lines if l.startswith('id')]), lost_total), '']
io.open('tmp/heal_diff_0914.txt', 'w', encoding='utf-8').write('\n'.join(head + lines) + '\n')
print('\n'.join(head[:2]))
print('\n'.join(lines[:40]))
