# -*- coding: utf-8 -*-
"""投入前の重複チェック（9/10版・9/8版をファイル名だけ引数にした写し）。

 ①枠0のエントリを外す（貸切公演など）
 ②正規化名が既存と一致する候補を洗い出し、(県, 公演日, 締切日, 発売日) まで比べて
   「同じ販売窓＝投入しない」「別の窓＝投入する（または既存へ足す）」に分ける。
   ＝[[feedback_capture_all_deadlines_on_add]] の判定方法。
🚨 部分一致で畳まない（「新日本フィル」が消える事故）＝完全一致だけ。

使い方: python tmp/dedup_0910.py tmp/built_0910.json
"""
import collections
import io
import json
...
SRC = sys.argv[1] if len(sys.argv) > 1 else 'tmp/built_0910.json'
OUT_MD = SRC.replace('.json', '_dedup.md')
OUT_FRESH = SRC.replace('.json', '_fresh.json')
...
for c in alive:
    ...
    if cw and cw <= exist_w:
        ...
        addto.append((c, hits, cw - exist_w))

o = io.open(OUT_MD, 'w', encoding='utf-8')
W = o.write
W('# 投入前の重複チェック（%s）\n\n' % SRC)
...
json.dump(fresh, io.open(OUT_FRESH, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('ビルド %d / 枠0 %d / そのまま投入 %d / 既存に無い窓あり %d / 投入しない %d'
      % (len(built), len(zero), len(fresh), len(addto), len(same)))
print('→ %s ／ %s' % (OUT_MD, OUT_FRESH))
