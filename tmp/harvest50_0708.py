# -*- coding: utf-8 -*-
"""7/8 新着50件 収集driver（複合bash禁止→python1本化）。
...
def run_harvest(lg, flt, tag):
    out = 'tmp/presale_%s_0708.json' % tag
    try:
...
# assign ids, build cand file
cands = []
for n, (tag, it) in enumerate(selected):
    cands.append({'newid': maxid + 1 + n, 'artist': it['artist'], 'urls': [it['url']],
                  '_srcgenre': tag})
json.dump(cands, open('tmp/cand50_0708.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
