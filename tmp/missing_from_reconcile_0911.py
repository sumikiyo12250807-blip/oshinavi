...
使い方: python tmp/missing_from_reconcile_0911.py tmp/reconcile_recheck_0911.txt
出力: 画面に一覧／tmp/missing_ids_0911.txt（カンマ区切り）
"""
import re, sys
...
for (i, n), ls in sorted(rows.items()):
    ...
    for l in ls[:4]:
        print('    ' + l)
open('tmp/missing_ids_0911.txt', 'w', encoding='utf-8').write(','.join(str(i) for (i, n) in sorted(rows)))
print('MISSINGを持つid %d件 → tmp/missing_ids_0911.txt' % len(rows))
