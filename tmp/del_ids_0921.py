...
**同じ3件で一致**した）＝id1904（配信〜10/4）／id6253（視聴券〜9/25）／id13937（アーカイブ〜9/27）
＋ id17356（新着プールの1件・公演が9/20で終わっている）を足す。

出力: tmp/del_ids_0921.txt（カンマ区切り）／tmp/del_list_0921.md（公演名＋確認用URL）
"""
import io, json, re

TODAY = '2026-09-21'
KEEP = {1904, 6253, 13937}
...
io.open('tmp/del_ids_0921.txt', 'w', encoding='utf-8').write(','.join(ids))

md = io.open('tmp/del_list_0921.md', 'w', encoding='utf-8')
md.write('# 削除した公演（2026-09-21 朝）\n\n')
md.write('公演が終わったもの **%d件**。残したのは配信が生きている3件'
         '（id1904 〜10/4／id6253 〜9/25／id13937 〜9/27）。\n\n' % len(rows))
...
md.close()
print('ids=%d  → tmp/del_ids_0921.txt / tmp/del_list_0921.md' % len(rows))
