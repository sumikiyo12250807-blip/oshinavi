...
・発売前＝tmp/sweep_presale_0911/*.json（rlsStatus=0102 / 0202）
・受付中＝tmp/rest0101_0910.json（9/9のスイープで残した2,255件のタネ。スイープはやり直さない）
ジャンル優先順＝①音楽 ②演劇/クラシック ③その他（[[feedback_harvest_genre_priority]]）。
🚨締切が近いものも落とさない（2026-09-07 にユーザーが締切4日先の縛りを外した）。
🚨同名の既存があるものは投入せず統合行きに回す。

使い方: python tmp/pick_presale_0911.py [件数(既定100)]
"""
import collections
import glob
import io
...
for p in sorted(glob.glob('tmp/sweep_presale_0911/*.json')):
...
# ② 足りなければ受付中の残りタネで埋める
rest = json.load(io.open('tmp/rest0101_0910.json', encoding='utf-8'))
...
json.dump([{'artist': c['artist'], 'urls': [c['url']], 'eventCd': cd}
          ...
          io.open('tmp/cand_0911.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
json.dump([{'eventCd': cd, 'artist': c['artist'], 'url': c['url'], 'lg': lg}
           for lg, cd, c, s in hold],
          io.open('tmp/merge_0911.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
left = [x for x in rest if x['eventCd'] not in {b[1] for b in batch}]
json.dump(left, io.open('tmp/rest0101_0911.json', 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1)
print('\n→ tmp/cand_0911.json（今回）/ tmp/merge_0911.json（統合行き %d件）/ '
      'tmp/rest0101_0911.json（受付中の残り %d件）' % (len(hold), len(left)))
