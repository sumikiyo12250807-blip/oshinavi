...
DAYS = {}
_d = _dt.date(2026, 9, 14)
while _d <= _dt.date(2026, 9, 20):
    DAYS['%d/%d/%d' % (_d.year, _d.month, _d.day)] = _d.isoformat(); _d += _dt.timedelta(days=1)
...
for iso, r in sorted(hits, key=lambda x: x[0]):
    ...
    if not has:
        miss.append((iso, r, e))
print('ぴあ lg=%s で9/14〜9/20発売の行 %d件 ／ OSHINAVIにその日の枠が無い %d件' % (LG, len(hits), len(miss)))
for iso, r, e in miss:
