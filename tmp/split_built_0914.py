# -*- coding: utf-8 -*-
"""組み立てた新着（built）を「既存に畳む」「新規で入れる」「人が見る」に仕分ける（読むだけ・2026-09-14）。
...
使い方: python tmp/split_built_0914.py <built.json> <cands.json>
出力: tmp/split_built_0914.txt ／ tmp/merge_built_0914.json ／ tmp/merge_cands_0914.json ／ tmp/new_built_0914.json
"""
import io
import json
import re
...
json.dump(merge_b, io.open('tmp/merge_built_0914.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
json.dump(merge_c, io.open('tmp/merge_cands_0914.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
json.dump(new_b, io.open('tmp/new_built_0914.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
head = '組み立て %d件 ＝ 畳む %d件 / 新規（部分一致の要確認を含む）%d件 / 複数一致 %d件' % (
    len(built), len(merge_b), len(new_b), sum(1 for l in lines if l.startswith('👀複数')))
io.open('tmp/split_built_0914.txt', 'w', encoding='utf-8').write(head + '\n' + '\n'.join(lines) + '\n')
print(head)
print('\n'.join(lines))
