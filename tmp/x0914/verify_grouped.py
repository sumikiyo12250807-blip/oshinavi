# -*- coding: utf-8 -*-
"""まとめ直した投稿のリストが、素材（tmp/x0914/material.md）の行を1つも落としていないか確かめる（読むだけ・9/14夜の便）。
素材の各行（時刻・名前・県・先行）を、投稿側の「時刻 名前／県・県…」に展開したものと突き合わせる。
まとめる規則（ALIAS・LINE）は tmp/x0913/group_lists.py と同じものを使う。
使い方: python tmp/x0914/verify_grouped.py
"""
...
posts = ''.join(io.open(f, encoding='utf-8-sig').read() + '\n' for f in sorted(glob.glob('tmp/x0914/post0[4-9].txt')))
