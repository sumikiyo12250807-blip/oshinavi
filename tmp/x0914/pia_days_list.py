# -*- coding: utf-8 -*-
"""ぴあの発売前一覧（指定ジャンル・先着0102/抽選0202）を全ページ読み、発売日が 9/15〜9/17 の行を全部出して、
OSHINAVIにその発売日の枠があるかを突き合わせる（読むだけ・9/14夜の便＝X素材の予約前の取りこぼし確認）。
使い方: python tmp/x0914/pia_days_list.py <lg>   例: 01=音楽 02=演劇 07=クラシック 06=イベント 03=スポーツ 04 05
...
DAYS = {'2026/9/15': '2026-09-15', '2026/9/16': '2026-09-16', '2026/9/17': '2026-09-17'}
...
for iso, r in sorted(hits, key=lambda x: x[0]):
    ...
    if not has:
        miss.append((iso, r, e))
out = ['ぴあ lg=%s で9/15〜9/17発売の行 %d件 ／ OSHINAVIにその日の枠が無い %d件' % (LG, len(hits), len(miss))]
for iso, r, e in miss:
    out.append('  %s %s | %s | %s | %s' % (iso, r.get('artist'), r.get('saletype'), r.get('venue'),
               ('登録あり id%s（その日の枠なし）' % e['id']) if e else ('未登録 ' + r['url'])))
io.open('tmp/x0914/pia_days_%s.txt' % LG, 'w', encoding='utf-8').write('\n'.join(out) + '\n')
print('\n'.join(out))
