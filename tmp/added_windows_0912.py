...
出力: tmp/added_windows_0912.txt（全件）／ tmp/push_sample_0912.txt（抜き取り・エージェントに渡す）
"""
import io
import json
import random
...
with io.open('tmp/added_windows_0912.txt', 'w', encoding='utf-8') as f:
    for r in rows:
        f.write('id%s | %s | %s | date=%s start=%s | %s\n' % r)
random.seed(912)
sample = random.sample(rows, min(N, len(rows)))
with io.open('tmp/push_sample_0912.txt', 'w', encoding='utf-8') as f:
    for i, name, ty, d, sd, u in sorted(sample):
        f.write('%s\t%s\t%s\n' % (i, u, name[:40]))
print('既存エントリに増えた（または書き換わった）枠 %d件 → tmp/added_windows_0912.txt' % len(rows))
print('抜き取り %d件 → tmp/push_sample_0912.txt' % len(sample))
