# -*- coding: utf-8 -*-
"""9/11夜のX投稿（明日9/12発売）の素材を、投稿1本ずつに割って1枚にする。Fableに渡すのはこれと X_SCRIPT.md だけ。
・明日9/12発売＝その投稿のジャンルを全部（同じ時刻・名前・県・先行の行は1行にまとめる）
・9/13・9/14＝その投稿のジャンルから箱の大きい順に5件、残りは「他にも◯件以上」（10の位で切り下げ・10未満はそのまま）
🚨「発売開始」＝ticket.startDate がその日のもの。新着プール（振り分け前）は出さない。売り切れ枠は出さない。
出力: tmp/x0912/material.md
"""
...
os.makedirs('tmp/x0912', exist_ok=True)
o = io.open('tmp/x0912/material.md', 'w', encoding='utf-8')
W = o.write
W('# 9/11夜のX投稿の素材（明日 9/12(土) 発売）\n\n')
