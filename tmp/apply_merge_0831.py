...
def key(t):
    ...
    return (t.get('type'),u)
reb={e['id']:e for e in json.load(io.open('tmp/_merged_0831.json',encoding='utf-8'))}
P='index.html'
shutil.copy(P,'index.html.bak_0831_merge')
src=io.open(P,encoding='utf-8',newline='').read()
m=re.search(r'(  const EVENTS = )(\[.*?\])(;)',src,re.S)
EVENTS=json.loads(m.group(2))
log=io.open('logs/merged_2026-08-31.md','w',encoding='utf-8')
log.write('# 2026-08-31 朝 統合（同名33件＋劇団四季/N響6件＋取りこぼし救済・追加のみ・置換なし）\n\n')
nent=nslot=ndate=0
for e in EVENTS:
    b=reb.get(e['id'])
    if not b: continue
...
# 🚨インデントを足さない。json.dumps(indent=2) が index.html の既存の形と同じ。
#   '  '+l を足すと全行が右に2つずれて差分が25万行になる（2026-08-31に踏んだ）。
out=src[:m.start(2)]+arr+src[m.end(2):]
if '\r\n' in src: out=out.replace('\r\n','\n').replace('\n','\r\n')
io.open(P,'w',encoding='utf-8',newline='').write(out)
log.write('\n合計 %d エントリ / +%d 枠 / 公演日が伸びた %d件\n'%(nent,nslot,ndate))
log.close()
print('統合 %d エントリ / 追加 %d 枠 / 千秋楽が伸びた %d件 → logs/merged_2026-08-31.md'%(nent,nslot,ndate))
