# -*- coding: utf-8 -*-
"""ぴあ受付中の音楽（9/22 sg×pf スイープ）で「売切」で組めなかった161件を、印付きの枠として組み立てる。
...
出力: tmp/x0922/built_onsale01_sold.json（1候補＝1URL・各ticketに url を付ける）
"""
...
SKIP_KEEP = set(int(x) for x in io.open('tmp/x0922/sold_ids.txt').read().split(','))
...
cands = [c for c in json.load(io.open('tmp/x0922/cands_onsale01.json', encoding='utf-8')) if c['newid'] in SKIP_KEEP]
out = []
for c in cands:
    e = B.build(c)
    if not e:
...
json.dump(out, io.open('tmp/x0922/built_onsale01_sold.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
