...
使い方: python tmp/x0922/audit_posts.py
出力: tmp/x0922/audit_posts.txt（報告）／tmp/x0922/audit_keywords.txt（引いた名前・外した名前）
"""
import datetime
...
io.open('tmp/x0922/audit_keywords.txt', 'w', encoding='utf-8').write(
    '引く名前 %d\n%s\n\n外した名前（公演名そのもの・長すぎる・短すぎる欧文）%d\n%s\n' % (
        len(kws), '\n'.join(kws), len(skipped), '\n'.join(skipped)))
print('引く名前 %d ／ 外した名前 %d → tmp/x0922/audit_keywords.txt' % (len(kws), len(skipped)))
pma.audit(kws, reg, excl, 5, 'tmp/x0922/audit_state.json', 'tmp/x0922/audit_posts.txt', prior=None, rls_from=None, today=TODAY)
print('→ tmp/x0922/audit_posts.txt')
