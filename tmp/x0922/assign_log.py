...
ids = [int(x) for x in io.open('tmp/x0922/assigned_ids.txt').read().split()]
h = io.open('index.html', encoding='utf-8').read()
ev = {e['id']: e for e in json.loads(re.search(r'  const EVENTS = (\[.*?\]);', h, re.S).group(1))}
with io.open('logs/assigned_2026-09-22_onsale.txt', 'w', encoding='utf-8') as f:
    f.write('# 2026-09-22 ぴあ受付中（音楽）272件の振り分け（ぴあのカテゴリを機械で写す・客観チェックで韓国5件をkpopに／里アンナ21696・中村滉己21698は保留）\n')
    n = 0
    for i in ids:
        e = ev.get(i)
        if not e:
        ...
        f.write('id%s\t%s%s\t%s\t%s\n' % (i, e.get('genre'), ('+' + '+'.join(e['extraGenres'])) if e.get('extraGenres') else '',
                                        e.get('name'), url))
        n += 1
print('logs/assigned_2026-09-22_onsale.txt に %d件' % n)
