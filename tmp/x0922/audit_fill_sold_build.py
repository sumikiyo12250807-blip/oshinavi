# -*- coding: utf-8 -*-
"""X投稿の総ざらい（9/22夜）で見つかった抜けのうち「売切」で組めなかった4件を、印付きの枠として組み立てる。
...
出力: tmp/x0922/audit_fill_sold.json（1候補＝1URL・各ticketに url を付ける）
"""
...
SKIP_KEEP = set(int(x) for x in io.open('tmp/x0922/sold_ids_fill.txt').read().split(','))
...
cands = [c for c in json.load(io.open('tmp/x0922/audit_fill_cands.json', encoding='utf-8')) if c['newid'] in SKIP_KEEP]
out = []
for c in cands:
    e = B.build(c)
    if not e:
...
json.dump(out, io.open('tmp/x0922/audit_fill_sold.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
