# -*- coding: utf-8 -*-
"""X投稿の素材出し＝9/6(明日)発売＋9/7・9/8(2〜3日後)発売の枠を、ジャンル別×時間順で並べる。
...
TARGETS = ['2026-09-06', '2026-09-07', '2026-09-08']
WD = '月火水木金土日'
...
_seen = set()
for e in EVENTS:
    # 🚨新着プール（genre="new"）は下書きの _genre で振り分ける。
    #   投稿は「1投稿＝1ジャンル」なので、"新着" という束では出せない。
    g = e.get('genre') or ''
    if g == 'new':
    ...
    for t in e.get('tickets', []):
        ...
        # 🚨同じ公演が別エントリに二重登録されていることがある（COUNTRY YARD／LAST ALLIANCE ほか）。
        #   投稿に同じ名前が2回並ぶので、(時刻, 名前, 県, 先行/一般) が同じ行は1つにする。
