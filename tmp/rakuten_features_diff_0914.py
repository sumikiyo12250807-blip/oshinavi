...
使い方: python tmp/rakuten_features_diff_0914.py
出力: tmp/rakuten_features_diff_0914.txt ／ tmp/rakuten_features_new_0914.json（これからの公演がある分＝build_rakuten_entries に渡せる形）
"""
import datetime
import io
import json
...
for i, u in enumerate(todo, 1):
    ...
    time.sleep(1.2)
io.open('tmp/rakuten_features_diff_0914.txt', 'w', encoding='utf-8').write('\n'.join(out) + '\n')
json.dump(fut, io.open('tmp/rakuten_features_new_0914.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('これからの公演がある %d本 / 過去・読めない %d本 → tmp/rakuten_features_diff_0914.txt' % (len(fut), len(out) - len(fut)))
for l in out:
    if l.startswith('🎯'):
        print(l)
