...
for g in GENRE_ORDER:
    try:
        d = json.load(io.open('tmp/presale_%s_0824.json' % g, encoding='utf-8'))
    except IOError:
        continue
    for it in d['new']:
        it['lg'] = g
...
json.dump(chosen, io.open('tmp/pick_0824.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
json.dump(merge, io.open('tmp/merge_cand_0824.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
json.dump(skip, io.open('tmp/skip_0824.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)

o = io.open('tmp/pick_0824.txt', 'w', encoding='utf-8')
for it in chosen:
    o.write('%s | 発売%s(あと%d日) | %s | %s | %s\n' % (
        it['lg'], it['rlsdate'], it['_days'], it['artist'], it['perfdate'], it['url']))
o.close()
print('→ tmp/pick_0824.json / tmp/pick_0824.txt')
