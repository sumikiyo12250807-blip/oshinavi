...
def key(t):
    ...
    return (t.get('type'),u)
reb={e['id']:e for e in json.load(io.open('tmp/rebuilt_merge_0828.json',encoding='utf-8'))}
P='index.html'
shutil.copy(P,'index.html.bak_0828_merge')
src=io.open(P,encoding='utf-8',newline='').read()
m=re.search(r'(  const EVENTS = )(\[.*?\])(;)',src,re.S)
EVENTS=json.loads(m.group(2))
log=io.open('tmp/merge_applied_0828.md','w',encoding='utf-8')
...
log.close()
print('統合 %d エントリ / 追加 %d 枠 / 千秋楽が伸びた %d件 → tmp/merge_applied_0828.md'%(nent,nslot,ndate))
