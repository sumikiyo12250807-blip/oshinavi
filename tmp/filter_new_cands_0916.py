# -*- coding: utf-8 -*-
"""スイープの「未掲載候補」から、本当に未登録のものだけを残す（読むだけ・2026-09-16 朝）。
...
出力: tmp/cands_<出力名>_0916.json（build_pia_entries にそのまま渡せる形）＋ .txt
"""
import glob
import io
import json
...
json.dump(out, io.open('tmp/cands_%s_0916.json' % TAG, 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
io.open('tmp/cands_%s_0916.txt' % TAG, 'w', encoding='utf-8').write('\n'.join(lines) + '\n')
print('読んだファイル %d本 / 本当に未登録の候補 %d件（名前×公演日で登録済み %d件を外した）→ tmp/cands_%s_0916.json' % (
