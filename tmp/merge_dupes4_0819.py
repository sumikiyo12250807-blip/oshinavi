...
BUILT = {b['id']: b for b in json.load(io.open('tmp/mergebuilt4_0819.json', encoding='utf-8-sig'))}
DROP = {int(k): v for k, v in json.load(io.open('tmp/mergedrop4_0819.json', encoding='utf-8')).items()}
...
for keep_id, built in sorted(BUILT.items()):
    ...
    if APPLY:
        keep['tickets'] = merged
        keep['date'] = newdate
        for f in ('venue', 'prefecture', 'dateLabel', 'name', 'artist'):
            if built.get(f):
                keep[f] = built[f]
...
bak = 'index.html.bak_0819_mergedupes4'
