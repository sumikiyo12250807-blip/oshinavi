# -*- coding: utf-8 -*-
"""7/9 期限切れ→販売中 変換(構築側)。reconcile MISSING の救済33件を機械再パース。
買える枠ゼロ(None)=売切/終了=削除候補。tmp/convert_0709.json出力。"""
import sys, json, re
sys.path.insert(0, 'tools')
import build_pia_entries as bpe
from build_pia_entries import build

IDS = [517,667,1051,1121,1240,1281,1707,1807,1808,1840,1841,1842,1843,1844,
       1845,1847,1849,1850,1893,1894,1895,1896,1929,1928,1989,1990,1991,
       2137,2142,2177,2188,2204,2229]
...
json.dump(out, open('tmp/convert_0709.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
