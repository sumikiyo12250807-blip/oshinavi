...
PIA_GENRE_MAP = {
    ...
    # 🚨2026-08-24 ユーザー判断で fes → jpop に変更。
    #   ぴあの「音楽その他」には、坂東玉三郎のシンフォニックコンサート・シネマコンサート・
    #   対談シリーズ・ライブハウスの周年イベントなど**フェスでないもの**が大量に入る。
    #   fes に落とすと「複数組＋屋外」という定義が壊れる（[[feedback_fes_definition]]）。
    #   受け皿として musicetc（表示「その他」）もあるが、**その他タブは誰も開かない＝埋もれる**ので使わない。
    #   音楽の大箱＝jpop に落として、探す人が音楽タブで見つけられるようにする
    #   （ユーザー「全部jpopでいいわ」／[[feedback_oshikatsu_first]]）。
    '音楽その他': ('jpop', None),
...
def _selftest():
    ...
    # 「イベントその他」は 2026-08-03 に engeki を下書きするよう変更（未収載のままだと
    # 名前fallbackで無言に倒れるため）。テストが旧仕様(None)のまま残って落ちていたので追随。
    assert genre_from_subcat('イベント', 'イベントその他', '第39期竜王戦第2局三島対局 前夜祭') == ('engeki', None)
    # 2026-08-04(第4弾)の回帰
