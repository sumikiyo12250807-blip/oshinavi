...
for it in items:
    ...
    # 🚨2026-09-14＝ビルダー側だけ直して、ここが古いままだった。「中止」「延期」「販売を終了致しました」
    #   「現在、お取り扱いしておりません。」「販売を見合わせています」はどれも is-before のまま出る＝
    #   単発確認でも「発売前」と読んでしまう。ビルダー（build_pia_entries.parse_cards）と同じ文言にそろえる。
    if re.search(r'(予定枚数|完売|売り?切|受付は?終了|販売終了|販売期間終了|終了しました|終了致しました|終了いたしました|結果発表|取扱なし|取り?扱いしておりません|見合わせ|貸切公演|中止|延期)', stat_text):
        state = '受付終了'
