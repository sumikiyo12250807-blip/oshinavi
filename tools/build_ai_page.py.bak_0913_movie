...
GENRE_LABEL = {
    ...
    # 「海外」はジャンルではなくエリア（index.html の PREFECTURE_TO_REGION に 台湾→kaigai）なのでここには入れない。
    "fanevent": "ファンイベント",
    ...
    # 2026-09-01 新設＝音楽グループの「伝統」（和太鼓・三味線・琵琶・箏・尺八・雅楽・民謡＝演奏を聴きに行くもの）。
    # 舞台の dento（歌舞伎・能・狂言・文楽・日本舞踊＝演じるのを観に行くもの）と表示名は同じ「伝統」。
    "hougaku": "伝統",
...
def status_text(ev, today):
    ...
    if t.get("saleEndUnknown") and t.get("startDate") and parse(t["startDate"]) <= today:
        ...
        return f'{emoji} {t["startDate"]}〜発売中（終了日は売り場に記載なし）', t["startDate"]
