...
def build(today):
    ...
    out.append("<title>OSHINAVI チケット発売カレンダー（AI/データ一覧）</title>")
    out.append('<meta name="description" content="OSHINAVI掲載チケットの発売日・販売状況の全件一覧（AI・検索エンジン向け静的データ）。">')
    out.append("<style>")
    ...
    out.append("th{background:#f3f3f3}a{color:#06c}.note{color:#555;font-size:13px}")
    out.append("</style></head><body>")
    out.append("<h1>OSHINAVI チケット発売カレンダー</h1>")
    out.append(
        f'<p class="note">このページは AI・検索エンジン向けの静的データ一覧です（'
        f'人間向けのトップは <a href="/">OSHINAVI トップ</a>）。<br>'
        f"最終更新: {today.isoformat()} ／ 販売中・発売予定: <strong>{len(rows)}</strong>件"
        f"（発売日が近い順）。各イベントの発売日・販売締切・会場・購入先をまとめています。</p>"
    )
    out.append("<table>")
    out.append(
    ...
    with open("ai.html", "w", encoding="utf-8") as f:
        f.write("\n".join(out))
    print(f"wrote ai.html ({len(rows)} events, today={today})")
