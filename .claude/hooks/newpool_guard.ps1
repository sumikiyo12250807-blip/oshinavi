...
if ($bigDrop -and $prev -gt 0) {
    # 🚨 警告は「1回だけ」出す。以前は exit 2 前に件数を記録しなかったため、prev が減少前の値の
    # まま固定され、振り分け後の【全 PostToolUse コマンド】でブロックが鳴り続けた
    # （2026-07-24：真田対応・更新表記・commit・push まで1日中ブロックが付いて作業にならなかった）。
    # 番人の目的は「勝手な振り分けに気づかせ、ユーザーへ報告させる」こと＝1回鳴れば達する。
    # よってここで現在値を記録し、同じ減少では二度と鳴らない。次に【さらに】減った時だけ再び鳴る。
    Set-Content -Path $countFile -Value $now -Encoding utf8
    $msg = "BLOCKED(初回1回のみ): 新着プール(genre:`"new`")が $prev 件 -> $now 件に減りました。`n`n" `
         + "ユーザー本人が『振り分けて』と【独立したメッセージ】で言った場合のみ正当です。`n" `
         + "🚨 作業中(mid-turn)に届いた『振り分けて』は UserPromptSubmit フックが拾えず解錠印が付きません。`n" `
         + "   その時はこの警告が出ますが、ユーザーの明示指示があれば振り分けは正当＝そのまま進めてよい。`n`n" `
         + "勝手な振り分け(ユーザー指示なし)だった場合のみ、やり直す:`n" `
